    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                db.child("rent_requests").child("from").child(LandingPage.curuser).child(owner.getText().toString().substring(8)).child(address.getText().toString().substring(10)).child("request").setValue(LandingPage.curuser+" has requested to rent the property at "+address.getText().toString());
                db.child("rent_requests").child("from").child(LandingPage.curuser).child(owner.getText().toString().substring(8)).child(address.getText().toString().substring(10)).child("status").setValue("Pending");
                db.child("rent_requests").child("to").child(owner.getText().toString().substring(8)).child(LandingPage.curuser).child(address.getText().toString().substring(10)).child("status").setValue("Pending");
                db.child("rent_requests").child("to").child(owner.getText().toString().substring(8)).child(LandingPage.curuser).child(address.getText().toString().substring(10)).child("request").setValue(LandingPage.curuser+" has requested to rent the property at "+address.getText().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
